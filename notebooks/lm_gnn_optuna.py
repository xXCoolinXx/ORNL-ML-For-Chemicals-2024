...
# # Set the GPU for this process based on its rank
# print(rank)
...
def objective(trial):
    ...
    (train_loader, val_loader, test_loader,) = hydragnn.preprocess.create_dataloaders(
        trainset, valset, testset, config["NeuralNetwork"]["Training"]["batch_size"] #pin_memory=False
    )
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, mode="min", factor=trial.suggest_float("scheduler_factor", 0.1, 0.8), patience=trial.suggest_int("scheduler_patience", 3, 10), min_lr=1e-6
    )
...
if __name__ == "__main__":
    ...
    # os.environ["OMPI_COMM_WORLD_SIZE"] = "1" #Deviously disable ddp
