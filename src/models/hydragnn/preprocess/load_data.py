...
def split_from_indices(dataset: list, idxs : list[int]):
    split = []

    for idx in idxs:
        split.append(dataset[idx])
...
def split_dataset(
    ...
    split_indices_path : str, # Deterministic save path
):
    trainset = []
    valset = []
    testset = []
    if os.path.exists(split_indices_path):
        print("Using existing QM9 index split. Delete the split and rerun this if you changed the settings.")
        train_indices, valid_indices, test_indices = torch.load(split_indices_path)

        trainset = split_from_indices(dataset, train_indices)
        valset = split_from_indices(dataset, valid_indices)
        testset = split_from_indices(dataset, test_indices)
    
    else: 
        print("Creating new index split...")
        if not stratify_splitting:
            ...
            print(perc_train, perc_val)
            train_size = int(perc_train * data_size)
            valid_size = int(perc_val * data_size)

            train_indices = indices[:train_size]
            valid_indices = indices[train_size:train_size + valid_size]
            test_indices = indices[train_size + valid_size:]

            trainset = [dataset[idx] for idx in train_indices]
            valset = [dataset[idx] for idx in valid_indices]
            testset = [dataset[idx] for idx in test_indices]

            torch.save([train_indices, valid_indices, test_indices], split_indices_path)
        else:
            print("Using stratified split")
            trainset, valset, testset, train_indices, valid_indices, test_indices = compositional_stratified_splitting(
                dataset, perc_train
            )
